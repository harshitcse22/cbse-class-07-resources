      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          <div className="flex items-center gap-1 flex-nowrap shrink-0 pt-0.5">
            <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mr-1 whitespace-nowrap shrink-0">Use in</span>
            {[
              ...
              <button key={name} onClick={() => randomPrompt && onOpenWithPrompt(url, randomPrompt.prompt)} className="cursor-pointer bg-transparent border-none p-0 shrink-0" title={name}>
        ...
        <button onClick={onGenerate} disabled={loading} className="w-full bg-[#1D4ED8] hover:bg-blue-700 text-white font-bold text-sm py-3 rounded-xl flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 transition-colors">
          <img src={`${base}Dice.svg`} alt="" className="w-5 h-5" /> {loading ? 'Generating...' : 'Generate Random Prompt'}
        </button>

        {randomPrompt && (
          <button onClick={onCopy} className="w-full mt-2 border border-gray-200 rounded-xl py-2.5 text-xs font-medium text-gray-500 flex items-center justify-center gap-1.5 cursor-pointer hover:bg-gray-50 transition-colors bg-white">
